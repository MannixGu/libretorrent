...
import static com.xm.cine.torrent.core.model.data.TorrentInfo.MAX_ETA;
...
import com.xm.cine.torrent.core.model.data.ReadPieceInfo;
import com.xm.cine.torrent.core.model.data.TorrentStateCode;
...
class TorrentDownloadImpl implements TorrentDownload {
    ...
    public FileInTorrent getFirstMediaFile() {
        return getAllMediaFile().stream().findFirst().orElse(null);
    }

    @Override
    public List<FileInTorrent> getAllMediaFile() {
        List list = new ArrayList<FileInTorrent>();
        ...
        for (int i = 0; i < tfs.numFiles(); i++) {
            ...
            if (fileMime != null && (fileMime.startsWith("video") || fileMime.startsWith("audio"))) {
                ...
                FileInTorrent fileInTorrent = new FileInTorrent(getFile(i), id, i, isDownloaded);

                list.add(fileInTorrent);
            }
        }
