...
import com.xm.cine.torrent.core.exception.NoFilesSelectedException;
import com.xm.cine.torrent.core.exception.UnknownUriException;
import com.xm.cine.torrent.core.model.AddTorrentParams;
...
import com.xm.cine.torrent.core.model.filetree.BencodeFileTree;
import com.xm.cine.torrent.core.model.filetree.FileNode;
...
import com.xm.cine.torrent.core.utils.BencodeFileTreeUtils;
...
public class AddTorrentViewModel {
    ...
    public AddTorrentViewModel(@NonNull Context context) {
        ...
        disposable.add(engine.observeNeedStartEngine()
                ...
                .subscribe((needStart) -> engine.doStart()));
...
//    }
    ...
    private static class TorrentDecodeTask extends AsyncTask<Uri, Void, Throwable> {
        ...
        protected Throwable doInBackground(Uri... params) {
            ...
            Uri uri = params[0];
            v.mutableParams.setUrl(uri.toString());
            try {
                switch (uri.getScheme()) {
                    ...
                    case Utils.HTTPS_PREFIX:
                        ...
                        byte[] response = Utils.fetchHttpUrl(v.context, uri.toString());
        ...
        private void readTorrentFile(Uri uri) throws IOException, DecodeException {
            ...
            ContentResolver contentResolver = v.context.getContentResolver();
    ...
    public boolean addTorrent(BiConsumer<AddTorrentParams, String> consumer) throws Exception {
        TorrentMetaInfo downloadInfo = info.get();
        if (downloadInfo == null)
            return false;
        ...
        consumer.accept(params, mutableParams.getUrl());
        ...
        try {
            Thread t = new Thread(() -> {
                try {
                    Torrent torrent = engine.addTorrentSync(params, false);
                } catch (Exception e) {
                    err[0] = e;
                }
            });
